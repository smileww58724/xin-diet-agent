...
public class DietAgentPrompt {
    ...
    public static String getSystemPrompt(User user) {
        ...
        prompt.append("""
            ...
            当用户询问饮食建议时，结合用户的个人信息和目标给出个性化建议。如需查询或记录用户的饮食与营养数据，可使用下方提供的数据工具。
            """);
    ...
    public static String getSystemPromptWithTools(User user) {
        return getSystemPrompt(user) + """
            ...
            【数据查询工具（function calling）】
            今日数据已注入，回答关于"今天"的问题时直接使用这些数据，不要调用工具；
            仅当用户询问其他日期或时间段（如"昨天""上周""这个月"）的饮食、营养数据时，才调用对应工具：
            - getDietRecords：查询某天的饮食记录列表
            - getNutritionSummary：查询某天的营养摄入汇总与目标进度
            - getWeeklySummary：查询从某日起 7 天的营养汇总
            - getUserProfile：查询个人资料与每日营养目标

            【饮食记录工具】
            当用户陈述吃了什么或喝了什么（如"我午饭吃了一碗牛肉面"）时，调用 addDietRecord 帮用户记录：
            - 热量与蛋白质/脂肪/碳水按常见食物营养估算，回复中必须注明"估算值"；
            - 一次提到多个食物时，分多次调用逐条记录；
            - 食物或份量不明确时（如"吃了个东西"），先询问清楚再记录，不要凭空编造；
            - 记录成功后告知用户记录了什么，并提示可在饮食记录页面修改或删除。
